...
public class GameManager : MonoBehaviour {
	...
	public UnityEngine.GameObject[] fx_prefabs;
	...
	void Update () 
	{
		...
		if ( Input.GetKeyDown("z") || Input.GetKeyDown("left") ){
            UnityEngine.GameObject.Find("UI-arrow-left").SendMessage("Go");
			index_fx--;
			if(index_fx <= -1)
				index_fx = fx_prefabs.Length - 1;
			text_fx_name.text = "[" + (index_fx + 1) + "] " + fx_prefabs[ index_fx ].name;	
		...
		if ( Input.GetKeyDown("x") || Input.GetKeyDown("right")){
            UnityEngine.GameObject.Find("UI-arrow-right").SendMessage("Go");
			index_fx++;
			if(index_fx >= fx_prefabs.Length)
				index_fx = 0;
			text_fx_name.text = "[" + (index_fx + 1) + "] " + fx_prefabs[ index_fx ].name;
